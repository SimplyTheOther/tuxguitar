...
public class TGBeatGroup {
	...
	public float getY1(TGLayout layout, TGNoteImpl note, int key, int clef){
		...
		//if (note.getMeasureImpl().getTrack().isPercussionTrack()) {
		//should provide same functionality with new fields and whatever
		if (layout.getSongManager().isPercussionChannel(note.getMeasureImpl().getTrack().getSong(), note.getMeasureImpl().getTrack().getChannelId())) {
			//Transpose percussion notes according to current note mapping
